// whatsapp-chatwoot-config — configuração Chatwoot UAZAPI
//
// Body: { instance_id: string, action: "get" | "update", config? }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { instance_id, action, config } = body;
    ...
    if (action === "get") {
      const res = await fetch(`${serverUrl}/chatwoot/config`, { headers: { token } });
      const data = await res.json().catch(() => ({}));
      return json({ success: res.ok, data }, res.ok ? 200 : 502);
    }
    if (action === "update") {
      if (!config) return bad("config obrigatório");
      const res = await fetch(`${serverUrl}/chatwoot/config`, {
        method: "PUT",
